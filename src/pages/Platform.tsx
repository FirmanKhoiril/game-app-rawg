...
import { getDataByPlatform } from "../api/fetch";
...
const Platform = () => {
  ...
  const { data, hasNextPage, fetchNextPage, isLoading, isFetching, isFetchingNextPage, isError, isSuccess } = useInfiniteQuery(["byPlatform", platform], ({ pageParam = 1 }) => getDataByPlatform(pageParam, platform), {
      ...
      {isLoading && isFetching ? (
        ...
        isSuccess && (
          <>
            ...
            {hasNextPage && (
              <button type="button" ref={ref} className={`py-2 px-2 m-auto block ${isFetchingNextPage ? "bg-transparent" : ""}`} onClick={handleNextPage}>
